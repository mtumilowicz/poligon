...
public class DatePeriodGenerator {
    ...
    public static DatePeriod generate() {
        Date dateFrom;
        return DatePeriod.DatePeriodFactory.get(dateFrom = DateGenerator.generate(), RandomUtils.nextBoolean() ? null : DateGenerator.generateAfter(dateFrom));
    }

    public static List<DatePeriod> generate(int howMany) {
        Preconditions.checkArgument(howMany > 1);
        LinkedList<DatePeriod> datePeriods = new LinkedList<>();
